...
drone_ma30 = drone['drone'].rolling(30).mean()

print(f"Drone: {n_samples} days | Battles: {n_battles}")
print(f"Date: {drone.index[0].date()} → {drone.index[-1].date()}")
...
fig1.savefig('drone_fig1_battle_gantt.png', dpi=250, facecolor='white', edgecolor='none')
print("  → drone_fig1_battle_gantt.png")

# ══════════════════════════════════════════════════════════════
# Fig 2 — ARIMA+GPR 预测 + 战役矩形叠加
#   左轴 = 无人机损失 (Actual + ARIMA fit + Hybrid + Forecast)
...
ax2.set_title('ARIMA+GPR Hybrid Forecast with Battle Context\n'
              '(Left axis = Drone Loss  |  Right axis = Battle Index — '
...
ax2.set_xlim(drone.index[0] - pd.Timedelta(days=10),
...
fig2.savefig('drone_fig2_arima_gpr_battles.png', dpi=250, facecolor='white', edgecolor='none')
print("  → drone_fig2_arima_gpr_battles.png")

# ══════════════════════════════════════════════════════════════
print(f"\n{'─'*50}")
print(f"Battles: {n_battles} | ARIMA{arima_order} | GPR: Matern(nu=1.5)")
print(f"Output: drone_fig1_battle_gantt.png")
print(f"        drone_fig2_arima_gpr_battles.png")
print(f"{'─'*50}")
print("Done.")
plt.show()
